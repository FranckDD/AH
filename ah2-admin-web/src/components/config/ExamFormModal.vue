        ...
        <div class="relative mx-auto w-full max-w-lg bg-white shadow-2xl rounded-xl border border-gray-100 p-6">
            ...
            <form @submit.prevent="handleSave" class="space-y-4">
                ...
                <div>
                    ...
                    <input v-model="form.nom" type="text" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('config.table_code') }}</label>
                        <input v-model="form.code" type="text" required :disabled="isEditing" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('config.table_category') }}</label>
                        <input v-model="form.categorie" type="text" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input 
                            ...
                            step="100" 
                            required 
                            class="w-full pl-4 pr-12 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 font-bold text-gray-800" 
                        />
                ...
                <div class="pt-6 flex justify-end space-x-3 border-t mt-4">
                    <button type="button" @click="$emit('close')" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition">
                        {{ t('config.cancel') }}
                    </button>
